...
export const LoginScreen: React.FC<LoginScreenProps> = ({
  loginName, loginPhone, loginEmail, loginPassword, setLoginName, setLoginPhone, setLoginEmail, setLoginPassword, onLogin, onRegister, isLoading, loginError
}) => {
  ...
  const isLoginDisabled = !loginPhone.trim() || !loginPassword.trim() || isLoading;
  const isRegisterDisabled = !loginName.trim() || !loginPhone.trim() || !loginEmail.trim() || !loginPassword.trim() || isLoading;
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-5 relative overflow-hidden">
      ...
      <div className="absolute bottom-40 right-0 w-40 h-40 bg-primary/3 rounded-full blur-[60px]" />
      ...
      >
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="glass-subtle rounded-xl p-3.5 flex items-start gap-3 border-primary/10">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="space-y-2.5">
          ...
          >
            {isLoading ? (
              ...
              <>
                {isRegisterMode ? 'Jisajili Sasa' : 'Ingia Sasa'}
                <ArrowRight size={16} />
              </>
            )}
          </motion.button>
